...
class PinionHypoid(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('motion_vector_in', shape=3, desc='input rotation/translation vector')
        ...
        self.add_output('motion_vector_out', shape=3, desc='output rotation/translation vector')
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['pos_out'] = pos_in + offset * place_dir + (md_pinion - l_pinion/2) * flow_dir_in + md_hypoid * flow_dir
        outputs['flow_dir_out'] = flow_dir # will be ignored by next component
        outputs['motion_vector_out'] = -1 * np.cross(place_dir, motion_vector_in)
        outputs['q_dot_out'] = (q_dot_in * N_pinion) / N_hypoid

        pinion_axis = np.abs(motion_vector_in)
        pinion_orthos = np.ones(3) - pinion_axis
        bb_in_min = pos_in - 0.5 * d_pinion * pinion_orthos - 0.5 * l_pinion * pinion_axis
        bb_in_max = pos_in + 0.5 * d_pinion * pinion_orthos + 0.5 * l_pinion * pinion_axis

        hypoid_axis = np.abs(outputs['motion_vector_out'])
        hypoid_orthos = np.ones(3) - hypoid_axis
        bb_out_min = outputs['pos_out'] - 0.5 * d_hypoid * hypoid_orthos
        bb_out_max = outputs['pos_out'] + 0.5 * d_hypoid * hypoid_orthos
