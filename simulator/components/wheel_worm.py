...
class WheelWorm(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        pos_out = pos_in + (r_worm + r_wheel) * place_dir + (l_worm/2) * flow_dir
        outputs['pos_out'] = pos_out
        outputs['flow_dir_out'] = flow_dir
        outputs['motion_vector_out'] = -1 * np.cross(place_dir, motion_vector_in)
        outputs['q_dot_out'] = (q_dot_in * N_wheel) / N_worm

        wheel_axis = np.abs(motion_vector_in)
        wheel_orthos = np.ones(3) - wheel_axis
        bb_in_min = pos_in - r_wheel * wheel_orthos - 0.5 * l_wheel * wheel_axis
        bb_in_max = pos_in + r_wheel * wheel_orthos + 0.5 * l_wheel * wheel_axis

        worm_axis = np.abs(outputs['motion_vector_out'])
        worm_orthos = np.ones(3) - worm_axis
        bb_out_min = pos_out - r_worm * worm_orthos - 0.5 * l_worm * worm_axis
        bb_out_max = pos_out + r_worm * worm_orthos + 0.5 * l_worm * worm_axis
