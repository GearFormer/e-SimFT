...
class RackPinion(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        pos_out = pos_in + l_rack * flow_dir_in + (r_pinion + h_rack/2) * place_dir
        ...
        rack_axis = np.abs(motion_vector_in)
        rack_h_axis = np.abs(place_dir)
        rack_w_axis = np.ones(3) - rack_axis - rack_h_axis
        bb_in_min = pos_in - 0.5 * h_rack * rack_h_axis - 0.5 * w_rack * rack_w_axis
        bb_in_max = pos_in + 0.5 * h_rack * rack_h_axis + 0.5 * w_rack * rack_w_axis

        pinion_axis = np.abs(outputs['motion_vector_out'])
        pinion_orthos = np.ones(3) - pinion_axis
        bb_out_min = pos_out - r_pinion * pinion_orthos - 0.5 * w_pinion * pinion_axis
        bb_out_max = pos_out + r_pinion * pinion_orthos + 0.5 * w_pinion * pinion_axis
