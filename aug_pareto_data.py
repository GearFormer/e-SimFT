...
if __name__ == "__main__":
    ...
    data = pd.read_pickle("esimft_data/pareto_test.pkl")
    data_size = len(data)
    ...
    for i in range(0, data_size):
        ...
        # obj_list = [price] + [calculate_volume(bb_min, bb_max)]
        # offset = 11
        # for j in range(0, len(obj_list)):
        #     data.loc[i, offset+j] = float(obj_list[j]) 
        #     print(obj_list[j])
        #     print(data.iloc[i])
        # input()
    ...
    df.to_pickle("esimft_data/pareto_test_obj.pkl")
