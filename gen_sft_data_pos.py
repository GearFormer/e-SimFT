...
if __name__ == "__main__":
    ...
    for i in range(0, data_size):
        ...
        for j in range(0, batch_size):
            ...
            with SuppressPrint():
                new_pos = simulator.run(input_data)["output_position"]

            # dataset.append((req_input, new_seq_idx_batch[j]))
            # break

            if np.linalg.norm((np.array(new_pos)-np.array(target_pos))) < 2e-02: 
                dataset.append((req_input, new_seq_idx_batch[j]))
                break
    ...
    df.to_pickle("esimft_data/sft_pos_train.pkl")
    df = pd.DataFrame(val_dataset, columns=['req_input', 'chosen_seq'])
    df.to_pickle("esimft_data/sft_pos_val.pkl")
